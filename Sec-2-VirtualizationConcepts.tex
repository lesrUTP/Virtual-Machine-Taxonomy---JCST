...
%		\includegraphics[width=4cm]{images/TheVirtualMachineMonitor_Popek1974.pdf}
        \includegraphics[width=4cm]{images/TheVirtualMachineMonitor_Popek1974.pdf}
		\vspace{-0.2cm}
		%\caption{Visión general de máquina virtual (VM) y monitor de máquinas virtuales (VMM) \footnotemark[2]{}. }
	...
	\footnotetext [2] {Based on the original figure of the virtual machine monitor from study, \textit{Formal Requirements for Virtualizable Third Generation Architectures} by Gerald J. Popek and Robert P. Goldberg in 1974.}
...
%	\begin{enumerate}
%		\item \textit {Equivalence}: To provide an execution environment for programs. 
%		This environment is identical to the original machine.
%		
%		\item \textit {Performance}: Programs that run in this environment show, in the worst case, only minor decreases in speed.
%		\item \textit {Resource Control}: The VMM is in complete control of the system resources.
%	\end{enumerate}
...
\begin{center}
 ...
 \vspace{2mm}
 \parbox[c]{8.3cm}{\footnotesize{Fig.1.~}  Virtual Machine Monitor (VMM) or Hypervisor}%\vspace*{.2mm}
\label{fig:VMM}
\end{center}
